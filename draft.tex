...
\end{equation}
\carlos{Note: Peschel only finds the entanglement energies to be equispaced in systems with nearest-neighbor hopping. The general picture is seems much more complex. So far it worked for the two-band models I considered because they effectively only have nearest-neighbor hopping, adding a $\sigma_0$ hopping for example breaks this. See in Fig.\ref{eh} that for $t'\neq 0$ this is not true anymore, and even in the $\nu=1$ phase there can be crossings.  }

\begin{figure}[h!]
\centering
\includegraphics[width=80mm]{eh1.pdf}
\caption{Entanglement energy spectrum for a cut with $t'=-2$.}
\label{eh}
\end{figure}
...
The reason expression~\eqref{loceq} simplifies so much for systems with nearest-neighbor hopping is that in this case it was found that all the eigenvalues of the ES are related to each other. The entanglement energies  (\carlos{Peschel always refers to them as entanglement energies or eigenvalues of the entanglement Hamiltonian. Entanglement spectrum he uses for the spectrum of the reduced density matrix, so it makes sense to use single-particle ES for the spectrum of CA}) come in pairs of left and right modes that are equispaced in the thermodynamic limit, so that the charges assigned are always $ \pm(+-+-+-...)$. In the case that the modes cross each other they do so for every pair, so that it just ammounts to a global minus sign in the charge assignment.
...
\carlos{check again becase I though that the kitaev chain had left-left right-right pairs, so in this case the entnsglement energies are not equispaced}
