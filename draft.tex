...
\begin{align}
\gamma =\left(\frac{1}{L}\sum_{s=1}^L (\chi_s \, \rm{mod} \, 1)\right)\, \rm{mod} \, 1,
\label{eq:inh_simple}
\end{align}
where
\begin{equation}
...
\subfloat[$t=1,t'=-2,s=0$]{\includegraphics[width=35mm]{4cdisorderES.pdf}
}
}\hspace{0mm}
...
\caption{Average position of the Wannier states in red and, slightly shifted, the localized entanglement eigenstates (\carlos{rename $C_A+\lambda C_A X C_A$?} in blue. In black we show the threshold $l_s$ obtained as the mean value of the two Wannier states close de $19$ and $20$) $L=80,\lambda=10^{-6}, t = 1, t'= 2, m=0$}
\label{fig:loc_fix}
\end{figure}
...
In order to ensure that we obtain localized entanglement eigenstates,  we diagonalize $C_A + \lambda C_A\hat{X}C_A$ instead of $C_A$.
For the whole system, one way of obtaining localized eigenstates of the full correlation function (\carlos{Hamiltonian?}) is to diagonalize $C\hat{X}C$, which results in localized Wannier states. $C$ commutes with this operator because it is a projector, but this is no longer the case for $C_A$. Far from the edges however, $C_A$ and $C$ are equivalent, so that the states at 1 will perfectly localize into Wannier states by adding the $\lambda C_A\hat{X}C_A/L$ term (\carlos{Note that the states at 0 or close to 0 don't localize ,but this is not an issue}). Note that in the thermodynamic limit the operator $\hat{X}$ acts as proportional to an identity for localized states. Therefore in this limit the localized eigenstates of $C_A$ with $\xi \neq 0,1$ are also eigenstates of this operator while their mixture is not. Threfore this operator breaks all degeneracies of $C_A$ and results in localized eigenstates.

One has to be careful when choosing the correct $\lambda$. A parameter $\lambda$ which is too small will fail to break the degeneracies and localize the eigenstates. On the other hand the $\lambda$ term will modify the eigenvalues and introduce an additional error. The eigenstates roughly spread over the whole chain so that we have $\expval{\hat{X}}_{i\alpha} \simeq i$. Therefore the error it introduces in $\chi$ is roughly
\begin{equation}
\lambda \sum_{\alpha=1,2}\left( \sum_{i=L/4+1}^{L/2}i-\sum_{i=1}^{L/4}i \right) = \frac{\lambda L^2}{8}
\end{equation}
Reescaling $\lambda$ to $\lambda/L^2$ we ensure that this error is roughly independent of system size and controlled only by $\lambda$. In Fig.\ref{lambdaL} we show this error in $\chi$ together with the average width for the eigenstates of the top half of the spectrum, for different system sizes. We see that indeed the error remains constant with system size. In this particualr case the degeneracies do not seem to induce a big error. However when we look at the average width we see three different regions. One region at low system size where the width is bounded by the system size and therefore the $\lambda$ term strongly modifies the eigenstates, which fail to localize. For incresing system size we see a second region where the system seems to localize and the width seems constant. And finally for further increasing system size a third region where the width increases again because the $\lambda$ term is too small to break the degeneracies and localize the eigenstates.
...
\subfloat[$\sum_{i/\xi_i>1/2} \bra{\psi_i}\sigma_x\ket{\psi_i}/L$]{\includegraphics[width=55mm]{lambdaL2.pdf}
}
}
\caption{$\lambda = 10^{-9}, t = 1, t'= 2, m=0$}
\label{lambdaL}
\end{figure*}

In Fig.\ref{lambdalambda} we keep the system size constant, and big enough so that we do not introduce a strong finite size error as seen above, and sweep $\lambda$ (from big to small). We see two regions. For big $\lambda$ the error made decreases proportional to $\lambda$ and the eigenstates remain localize, until some point were $\lambda$ is so small that similar to what hapened in the third region above the eigenstates fail to localize. In this case this leads to a wrong classification of the midgap modes which results in a completely wrong $\chi$.

Note that there is a lower bound to $\lambda$ given by the numerical accuracy, $\lambda_{\rm min} \sim 10^{-13} L^2$. Since the system has to be much greater than the correlation length in order for it to localize into Wannier states, we have $\lambda_{\rm min} \sim 10^{-13} l_{\rm corr}^2$, which will set the minimum error we can expect, as it scales only with the $\lambda$ parameter. (\carlos{Not 100\% sure about this statement. Specially how it works in relation to the system with interphase and zero-modes.})

For inhomogeneous systems the story is slightly different. Due to the cut-average a wrong assignment of the bulk modes leads to an error of order $\mathcal{O}(1/L)$. In Fig. \ref{lambdainhlambda} we have the error made in $\chi$ and the maximum width of the eigenstates for different values of $\lambda$ and system size $L$. For each system size we observe, similar as before, a region were the system localizes followed by a region where the effect of the $\lambda$ term is too small and the system fails to localize. The only difference being that due to the cut-average a wrong assignment of the bulk modes leads to an error of order $\mathcal{O}(1/L)$. Note that there is an optimal value for the system size that allows to choose the lowest $\lambda$. In Fig \ref{lambdainhL} we see the same results for a system with $\lambda=10^{-8}$, near the minimum allowed where the system still localizes for some $L$. We see, as in the homogeneous case, that there are three regions. One with a finite-size effect, a second one were the system localizes and a third one where $\lambda/L^2$ is too small and the system delocalizes again. 

\begin{figure*}[h!]
\centering
\makebox[0pt]{
\subfloat[${\rm Log}_{10}(\chi - \gamma)$]{
  \includegraphics[width=55mm]{lambdalambda1.pdf}
}
\subfloat[$\sum_{i/\xi_i>1/2} \bra{\psi_i}\sigma_x\ket{\psi_i}/L$]{\includegraphics[width=55mm]{lambdalambda2.pdf}
}
}
\caption{$L=80, t = 1, t'= 2, m=0$}
\label{lambdalambda}
\end{figure*}
...
\makebox[0pt]{
\subfloat[${\rm Log}_{10}(\chi - \gamma)$]{
  \includegraphics[width=55mm]{lambdainherr.pdf}
}
\subfloat[$\sum_{i/\xi_i>1/2} \bra{\psi_i}\sigma_x\ket{\psi_i}/L$]{\includegraphics[width=55mm]{lambdainhloc.pdf}
}
}
\caption{$L=40,80,120,160,200; t = 1, t'= 2, m=0$}
\label{lambdainhlambda}
\end{figure*}
...
\subfloat[$\sum_{i/\xi_i>1/2} \bra{\psi_i}\sigma_x\ket{\psi_i}/L$]{\includegraphics[width=55mm]{lambdaLinhloc.pdf}
}
}
\caption{$\lambda=10^{-8}, t = 1, t'= 2, m=0$}
\label{lambdainhL}
\end{figure*}
