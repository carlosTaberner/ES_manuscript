...
\makebox[0pt]{
\subfloat[$\kappa_i$]{
  \includegraphics[width=35mm]{cutavgwrongkap.pdf}
}
\subfloat[$t=1,t'=-2,m=1$]{\includegraphics[width=35mm]{cutavgwrong.pdf}
}
}
...
We have shown two cases were using expression \ref{eq:inh_simple} leads to the correct result. The average of a circular quantity is in fact not uniquely defined, and the average we do in Eq.\ref{eq:inh_simple}, which equates to averaging the valus in $(0,1)$ around $1/2$ (\carlos{is this clear?}), is just one choice that works in this cases. However in Fig.\ref{fig:simple_inh}.f this cut-average fails to give the Zak phase. This happens because by taking $\chi$ modulo 1 we have thrown out relevant information.
To avoid this issue we can instead take the cut-average 
\begin{align}
