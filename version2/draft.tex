...
\date{\today}
\begin{abstract}
The bulk polarization is a $\mathbb{Z}_2$ topological invariant for systems in one dimension with chiral or particle-hole symmetries. It is therefore related to the number of topological edge modes by the bulk-boundary correspondence. We show that the bulk polarization can always be decoded from the single-particle entanglement spectrum, even in the absence of symmetries that quantize it. From this relation the bulk-boundary correspondence can be derived in the presence of chiral and particle-hole symmetries. We use the bulk polarization to compute Chern numbers in 1D and 2D systems and show their relation to the entanglement spectrum. Furthermore we propose an alternative bulk polarization that can carry more information about the surface spectrum than the conventional one and can simplify the calculation of Chern numbers. 
\end{abstract}
...
It is not known, whether there exists a method to obtain the full topological content from the ground state. 
...
For \emph{non-interacting} topological insulators/superconductors the ES for (gapped) periodic systems can be computed very efficiently, using methods developed by Peschel and others~\cite{Peschel2003}. The ES in these systems is equivalent to the band-flattened energy spectrum of the corresponding system with open boundaries~\cite{Fidkowski2010entanglement}. The same correspondence was also found for closely related gapless systems~\cite{Matern2018entanglement}. However, even for non-interacting systems, it is unclear which information (beyond the  `edge' spectrum) is encoded in the entanglement spectrum. In this manuscript, we show that the information about the Zak phase, or the bulk polarization, can be completely decoded from the entanglement spectrum and provide a simple formula to do so. 
We also show how to extend this method to compute the Chern number directly from the entanglement spectrum. In obtaining these topological invariants, the Zak phase and the Chern number, directly from the entanglement spectrum the \emph{bulk-boundary correspondence} is seen in an explicit way.
...
This demonstrates that there should be a direct relation between the single-particle entanglement energies and the Zak phase, although deriving that relation turned out to be non-trivial. A particular limit of our result for homogeneous (translationally invariant) systems~\eqref{eq:chi_integrable} was derived by Ryu and Hatsugai for a fully dimerized SSH chain~\cite{Ryu2006}. 
...
The bulk polarization is defined in $\mathbb{R}$ mod 1, which presents challenges when computing changes in the polarization, which are defined  in $\mathbb{R}$. We show how, using our method, one can define an alternative bulk polarization computed in the equivalent open chain that is defined in $\mathbb{R}$. The bulk polarization obtained this way can give more information about the edge spectrum than the usual one, and can be used to simplify the computation of Chern numbers. We also discuss the relation between this alternative bulk polarization and another topological invariant known as the trace index \cite{Alexandrinata2011}.

\emph{Outline of the paper}: In section II we introduce the model used throughout the paper and we discuss its phase diagram. In section III we introduce the polarization, the different ways one can define it and its relation to the Zak phase. In section IV we introduce the entanglement spectrum and set the notation that will be used. In section V we introduce the method and show how it reproduces the Zak phase for both systems with and without translational invariance. In section VI we explain how the method applied to the opened chain allows us to compute Chern numbers for systems both in 1D and 2D.%and winding numbers 
...
This is a simple model which nevertheless supports a rich phase diagram containing topological phases with winding numbers up to $\nu =2$. 
...
This is shown in figure~\ref{fig:zero_E_modes}.b, where one clearly sees that the zero modes below $m=1$ are split. 
...
Figure~\ref{fig:zero_E_modes}.c and d show the edge spectrum for class AI and A, respectively. In both cases, the edge modes are split from zero for all values of $m$.
...
\caption{Edge spectrum along the dotted path (i.e. $t=1,t'=-2$) of Fig.~\ref{fig:bdi_phase_diagram} for different values of $\kappa$ and $\kappa'$.}
...
In Table \ref{fig:class_table} we summarize the different classes that are hosted by this model together with the correspondent symmetries and topological invariants in 1D. 
...
\end{tabular}
\caption{Different Cartan classes hosted by the model in Eq.\ref{bdi_model}, with the correspondent symmetries and topological invariants in 1D.} 
...
\end{align}
which makes $\ket{\tilde{\Psi}^\Phi}$ fully periodic in $\Phi$. We can now define the polarization
\begin{equation}
\tilde{\mathcal{P}} = \int_0^{2\pi}\frac{d\Phi}{2\pi} i\bra{\tilde{\Psi}^\Phi}\partial_\Phi \ket{\tilde{\Psi}^\Phi},
\label{eq:p_tilde}
\end{equation}
which is related to the current flowing through this bond \cite{Watanabe2018}. This polarization is related to the homogeneous polarization by 
\begin{equation}
...
Eq.~\eqref{eq:xi_eps} implies that the full information of the \emph{many-body} ES (MBES) is contained in the single-particle spectrum of the reduced correlation matrix. 
...
Let us first review previous results on the relation between the entanglement occupancy spectrum and the Zak phase. 
For symmetry-protected topological systems in one dimension, the quantized Zak phase is zero whenever there is an even number  of eigenvalues per vitual edge at $\xi = 1/2$, and $\pi$ when the number is odd~\cite{Peschel2008}. 
...
Huang and Arovas considered a two-dimensional Chern insulator and noted that there was a similarity between the Wannier centers (and therefore the Zak phase) and the `virtual edge mode' that connects the EOS values at 0 to those at 1~\cite{Huang2012,Huang2012-2}. 
...
In the latter case, there are several terms in Eq.~\eqref{eq:chi_integrable} with comparably large contributions and, consequently, the Zak phase deviates considerably from the 'virtual edge modes'. % This last sentece is not entirely ok. They observe flow in the zak phase that correspond s to flow in the EOS. 


\subsubsection{Systems with equispaced entanglement energies}
...
In the special case where there is only one eigenvalue per edge that is $\neq 0,1$ in  the EOS, like in the fully dimerized limit of the SSH chain, we recover the equality between one of the two midgap eigenvalues and the Zak phase found in Ref.~\cite{Ryu2006}. 
...
For small $m$, there are two dominant midgap eigenvalues, and the Zak phase follows the lower one very closely, see figure~\ref{huang}.b.
When this midgap eigenvalue approaches  0, the contribution from the other modes becomes more relevant and the Zak phase starts to deviate from the midgap eigenvalue, see figure \ref{huang}.c.


\begin{figure}[t]
\centering
\includegraphics[width=86mm]{fig3comp.pdf}
\caption{In (a) and (b)  we show the EOS (black), $\chi$ (green) and $\tilde{\mathcal{P}}^{\rm Bloch}$(red) for a cut in the phase diagram through the $\nu = 1 \rightarrow \nu = 0$ transition (dashed line in~\ref{fig:bdi_phase_diagram}), $t = 1,t'=0$, for parameters  (a) $\kappa =\kappa'=0$ and (b) $\kappa = 0.3, \kappa'=0$. Computed for $L=40$. In (c) we zoom in a region of the plot in (b) to show the splitting between $\tilde{\mathcal{P}}^{\rm Bloch}$ and the eigenvalue closest to $\xi=1/2$. In (d) we show a scaling plot of the difference between $\chi$ and $\tilde{\mathcal{P}}^{\rm Bloch}$ for parameters $t=1,t'=0,m=1$, $\kappa = 0.3, \kappa'=0$ and increasing system size to show that the difference vanishes in the thermodynamic limit. }
\label{huang}
\end{figure}

\subsubsection{General case}

In general we find that it is only the eigenstates that localize on the left edge that should contribute to $\chi$. The inclusion of bulk modes is irrelevant since they have $\xi = 0,1$. We can then compute
\begin{equation}
\chi = \sum_{\mu \in L} \xi_\mu \, {\rm mod} \, 1. 
\label{eq:chi_left}
\end{equation}
the sum is performed over the subspace $L$ formed by the eigenstates with $\expval{\hat{x}}<L/4$, where $L/4$ is the middle point of region $A$. It does not matter how we choose the eigenvalues we sum over as long as we do not include the ones related to the right edge with $\xi$ away from $1$. In practice only a few eigenvalues give a significant contribution to $\chi$. 
...
In figures \ref{2}.a and \ref{2}.b, we compute the EOS for varying $m$ at  $t=1$, $t'=2$, indicated by the solid line  in the phase-diagram in Fig.~\ref{fig:bdi_phase_diagram}, in the presence of two different symmetry breaking terms: (a) $\kappa =0.3$, $\kappa'=0$ and (b) $\kappa=0$, $\kappa'=0.3$.  
...
The idea is that in the process of opening the chain, which can be done adiabatically, all eigenvalues that are related to the right virtual edge should get pushed to $\xi=0,1$. As a result, the sum of all eigenvalues of $C_{A,o}$ is equal, modulo 1 and in the thermodynamic limit, to $\chi$ (See Eq.~\eqref{eq:chi_left}). The advantage of using $\tpo$ is that the contribution from the eigenvalues far from the virtual edge is always constant, as opposed to the case of $\chi$ where this is only true when taken modulo 1. The consequence is that for paths in parameter space, $C_\lambda$, which are gapped, i.e. no states cross the Fermi energy, $\tpo (\lambda)$ is defined in $\mathbb{R}$, such that the total change in $\tpo$ along this path can be obtained knowing $\tpo$ only in the initial and final states, $\Delta \tpo ({C_\lambda}) = \tpo(\lambda_f) - \tpo(\lambda_i)$. Note, however, that $\tpo$ is still gauge-invariant only when taken modulo 1. 
...
\end{equation}
so $\tpo(-\kappa) = L-\tpo(\kappa)$. Unless $\tpo(0) = L/2$, which is the result expected for a trivial insulator, the result will be discontinuous at $\kappa = 0$. We can see this in Fig.\ref{ssh_chern}.a where we show $\tpo$ in the parameter space $(m,\kappa)$. The presence of the zero-mode for $m<1$ appears in $\tpo$ as a branch cut. The relation between the zero-modes and $\Tr[C_{A,o}]$ was already observed in the context of Chern insulators \cite{Alexandrinata2011}. This branch cut is in fact not an artifice of the open chain trick, but the result of the degenerate point at $(m=1,\kappa=0)$ having an associated Chern number $C=1$ \cite{Asboth2016}.
...
\end{align}
which results in $+1$ when $m<1$ and $0$ when $m>1$, as it is known for the SSH chain. The only contribution to the Chern number comes from the discontinuity of $\tpo$ at the branch cut. In general the Chern number over a loop is equal to the sum of the discontinuities of $\tpo$ across the branch cuts. 


\begin{figure}[t]
\includegraphics[width=86mm]{fig5comp.pdf}
\caption{Polarization $\tpo-L/2$ computed for $t=1,\kappa'=0$. (a) and (b) are computed for $L=40$ sites and parameters $t'=0$ and $t'=-2$, respectively. $\tpo$ presents branch cuts that indicate the presence of zero-energy modes. $\tpo$ is able to differentiate between the phases $\nu=2$ and $\nu=0$. (c) is computed for $t'=2$, with an added disorder to $t$ and $m$ with strength $W=3$, and $\kappa$ is applied only at the edges. We compute it for $L=400$, as the disorder increases the correlation length. The disorder opens a region with $\nu=1$ as it has been observed in \cite{Song2014} }
	\label{ssh_chern}
\end{figure}
...
%			\includegraphics[width=45mm]{fig61.pdf}
%		}
%		\subfloat[$C $]{
%			\includegraphics[width=45mm]{fig62.pdf}
%		}		
%	}\hspace{0mm}
%	
%	\makebox[0pt]{
%		\subfloat[${\rm Log}_{10} (C \, {\rm mod} \, 1)$ ]{
%			\includegraphics[width=45mm]{fig63.pdf}
%		}
%	}
...
\subsubsection{Second neighbor hopping, $t' \neq 0$}
...
Note , however, that there are many ways of breaking chiral symmetry. Replacing $\kappa$ in Eq.~\eqref{bdi_model} by $\kappa_i=\kappa (-1)^i$, the zero-energy modes split in such a way that $\tpo$ is continuous at $\kappa=0$. It cannot differenciate between the phases $\nu=0$ and $\nu=2$, and a loop like the one discussed above results in a Chern number $C=0$.
...
&m_i = m + W \omega'_i
\end{align}
where $\omega_i$ and $\omega_i'$ are selected from a uniform distribution with range $[-1/2,1/2]$. For very strong disorder where the gap fills with states $\tpo$ will present many branch cuts as we approach the limit $\lim_{\kappa \rightarrow 0}$, which makes it unpractical. In order to avoid it we break chiral symmetry locally in the edge of the open chain so it only affects the edge modes. If any of the other states cross $E=0$ they do it in  chiral-symmetric pairs such that $\tpo$ is not affected by it. We show $\tpo$ in Fig.\ref{ssh_chern}.c using the local symmetry breaking term for a strong disorder, $W=3t$, where the gap is indeed filled with states. We see that it works just as in the case without disorder, where now due to the disorder a region of $\nu=1$ opens up \cite{Song2014}. The main difference is that since we break chiral symmetry only for the edge modes, $\tpo$ is quantized to multiples of $1/2$.

%In Fig.\ref{winding}.(b) we show $C$ for the continuous line in the phase diagram and increasing disorder strength. The Chern number as defined in \eqref{eq:Chern_number} indeed provides a topological invariant which can be compared with the winding number obtained in reference \cite{Song2014} for the same system. Note that for large enough systems the spectrum of $C_A$ is not affected by the local chiral symmetry breaking term other than the occupation of the zero-energy modes such that $C$ is still quantized for each disorder configuration, however near the transitions it is smoothen by the disorder average. We show the error in the quantization of $C$ in figure \ref{winding}.(c) . \carlos{Fig.6.a seems important to discuss how with local symmetry braking term the relation between the chern number and the number of zero modes still remains. Maybe it would be enough to show a cut of 6.b }
...
We have shown how $\tpo$ simplifies the computation of Chern numbers in 1D chains. This can be generalized to 2D systems. Consider the following model for a Chern insulator
\begin{align}
...
\end{align}
We can treat the Hamiltonian with elements $H_{i\alpha,j\beta}(k_y)$ as a 1D Hamiltonian with an extra parameter $k_y$ and compute $\tpo$, or equivalently $A_\Phi(\Phi,k_y)$, as in last section. But first lets analyze the entanglement occupancy spectrum of the system with PBC, shown in Fig.\ref{chern_insulator}.a and b for $m=1,3$ respectively. For $0<m<2$ the EOS presents two edge modes that connect the occupied and empty bands that cross $\xi=1/2$ at $k_y=0$, in this region the occupied bands have a Chern number $C=+1$. For $2<m<4$ there are also edge modes but they cross $\xi=1/2$ at $k_y=\pi$ and the system has $C=-1$. For $m>4$ the system is a trivial insulator with $C=0$. When we open the chain the eigenvalue of one of the virtual edge modes will be pushed to $\xi=1,0$ as seen in figure~\ref{chern_insulator}.a and b. It will have a jump whenever the correspondent edge-mode crossed zero energy, going between $\xi=1$ and $\xi=0$, i.e. the number of eigenvalues at $\xi = 1$ is not constant across $k$. In this model, due to the symmetries, this is only possible at $k_y=0,\pi$. 

\begin{figure}[t]
	\centering
	\makebox[0pt]{
		\subfloat[EOS for $m=1$]{
			\includegraphics[width=45mm]{fig71.pdf}
		}
		\subfloat[EOS for $m=3$]{
			\includegraphics[width=45mm]{fig72.pdf}
		}
	}\hspace{0mm}
	
	\makebox[0pt]{
		\subfloat[$\tpo-L/2$]{
			\includegraphics[width=45mm]{fig73.pdf}
		}
		\subfloat[$\tilde{\mathcal{P}}$ mod 1]{
			\includegraphics[width=45mm]{fig74.pdf}
		}
	}
%	\hspace{0mm}
%	
%	\makebox[0pt]{
%		\subfloat[$\tpo-L/2$]{
%			\includegraphics[width=45mm]{fig75.pdf}
%		}
%		\subfloat[$\tilde{\mathcal{P}}$ mod 1]{
%			\includegraphics[width=45mm]{fig76.pdf}
%		}
%	}
%	
	\caption{(a) and (b) EOS. The evolution of the edge state towards $\xi=1,0$ as the chain is opened adiabatically is shown in two intermediate points. (c) $\tpo$ and (d) $\tilde{\mathcal{P}}$. Computed for the Chern insulator given by the model in Eq.~\eqref{eq:chern_model}. }
...
% This example might seem insufficient as the Chern number is also easy to identify from the plot of $\tilde{\mathcal{P}}$ as well but in systems with higher Chern numbers $\tilde{\mathcal{P}}$ will wind around the circle several times which will make this more difficult. We exemplify this in figures \ref{chern_insulator}.(e) and (f). To obtain these we change the nearest neighbor hoping in the $x$ direction into a third neighbor hopping, $\delta_{i,j+1} \rightarrow \delta_{i,j+3}$. This results in the same phase diagram with the Chern number, and the number of edge modes, multiplied by 3. As we see, while the plot of $\tpo$ remains simple, $\tilde{\mathcal{P}}$ becomes more complex, with the appearance of additional branch cuts. 

%
%   \carlos{Figures 7.e and f are computed for the same system where instead of nearest-neighbor hopping in the x direction we use a 4th neighbor hopping ($\delta_{i,j+1} \rightarrow \delta_{i,j+4}$), to illustrate the challenges of computing the Chern number when the integrand is only defined modulo 1. This is similar to the issues we had when doing the cut-average. One needs to identify whenever $\tilde{\mathcal{P}}$ (or $\chi$) crosses 1 or 0 to compute the derivative and integral properly.
%
...
It was previously known that the Zak phase, or the geometric phase for $U(1)$ flux insertion, was encoded in the many-body entanglement spectrum. Here we have shown how it is encoded in the single-body entanglement spectrum. This relation is simpler than the one involving the MBES and it provides more insight into the ES and EOS, as well as it provides another simple method for computing the Zak phase for systems without translational invariance. Based on this method we show that one can obtain a polarization that is defined in the real axis. Although it is not gauge invariant it provides additional information and also simplifies the method for computing Chern numbers for systems in 1D without translational invariance or systems in 2D with translational invariance in only one direction.
...
In this appendix, we review  how to compute the Zak phase from the Schmidt decomposition~\cite{Zaletel2014}and relate it to our own results.
